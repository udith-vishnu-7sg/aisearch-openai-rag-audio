# VoiceRAG: An Application Pattern for RAG + Voice Using Azure AI Search and Azure OpenAI Models

[![Open in GitHub Codespaces](https://img.shields.io/static/v1?style=for-the-badge&label=GitHub+Codespaces&message=Open&color=brightgreen&logo=github)](https://github.com/codespaces/new?hide_repo_select=true&ref=main&skip_quickstart=true&machine=basicLinux32gb&repo=860141324&devcontainer_path=.devcontainer%2Fdevcontainer.json&geo=WestUs2)
[![Open in Dev Containers](https://img.shields.io/static/v1?style=for-the-badge&label=Dev%20Containers&message=Open&color=blue&logo=visualstudiocode)](https://vscode.dev/redirect?url=vscode://ms-vscode-remote.remote-containers/cloneInVolume?url=https://github.com/Azure-Samples/aisearch-openai-rag-audio)

This repo contains an example of how to implement RAG support in applications that use voice as their user interface, powered by Azure OpenAI models (gpt-realtime-mini for transcription and text-to-speech, gpt-5-mini for chat). We describe the pattern in more detail in [this blog post](https://aka.ms/voicerag), and you can see this sample app in action in [this short video](https://youtu.be/vXJka8xZ9Ko).
...
* **Voice interface**: The app uses the browser's microphone to capture voice input, and sends it to the backend where it is transcribed using Azure OpenAI gpt-realtime-mini.
* **RAG (Retrieval Augmented Generation)**: The app uses the Azure AI Search service to answer questions about a knowledge base, and sends the retrieved documents to gpt-5-mini to generate a response.
* **Audio output**: The app synthesizes the response text using Azure OpenAI gpt-realtime-mini and plays it using the browser's audio capabilities.
* **Citations**: The app shows the search results that were used to generate the response.

### Architecture Diagram

The frontend receives the audio input, sends that to the Python backend which uses gpt-realtime-mini for transcription, gpt-5-mini for chat completion with RAG tools for searching Azure AI Search, and gpt-realtime-mini for speech synthesis.
...
The deployment process has **two required steps**:
...
3. (Optional) This is the point where you can customize the deployment by setting azd environment variables, in order to [use existing services](docs/existing_services.md) or [customize the voice choice](docs/customizing_deploy.md).

4. Provision the infrastructure and deploy the application code:
   ...
   * **Note**: This step creates the Azure OpenAI service but does NOT deploy any models. Models must be deployed in Step 2.

### Step 2: Deploy Models

**After `azd up` completes successfully**, deploy all required models using the provided script:

```bash
./scripts/deploy_models.sh
```

Alternatively, you can use the Python script or Azure CLI directly (see [Manual Model Deployment Guide](docs/manual_model_deployment.md)).

This script deploys:
- **gpt-realtime-mini** - For transcription and text-to-speech
- **gpt-5-mini** - For chat completions
- **text-embedding-3-large** - For embeddings

The script automatically reads environment variables from your azd environment. If it can't find them, it will prompt you for the OpenAI service name and resource group.

**Alternative**: You can also deploy models manually via Azure Portal. See [Manual Model Deployment Guide](docs/manual_model_deployment.md) for instructions.

### Step 3: Use the Application

After both steps complete, you will see a URL printed to the console from `azd up`. Navigate to that URL to interact with the app in your browser. To try out the app, click the "Start conversation button", say "Hello", and then ask a question about your data like "What is the whistleblower policy for Contoso electronics?" 
...
* Azure OpenAI: Standard tier, gpt-realtime-mini (for transcription and speech synthesis), gpt-5-mini (for chat), and text-embedding-3-large (for embeddings). Pricing per 1K tokens used. [Pricing](https://azure.microsoft.com/pricing/details/cognitive-services/openai-service/)
...
* [Azure OpenAI Audio Transcription Documentation](https://learn.microsoft.com/azure/ai-services/openai/whisper-quickstart)
* [Azure OpenAI Chat Completions Documentation](https://learn.microsoft.com/azure/ai-services/openai/how-to/chatgpt)
* [Azure OpenAI Text-to-Speech Documentation](https://learn.microsoft.com/azure/ai-services/openai/text-to-speech)
